  ...
  return (
    <div className="p-6 flex flex-col min-h-[500px] pb-10 bg-[#151212] text-white rounded-3xl">
      ...
      <div className="bg-[#141414] rounded-2xl border border-white/10 p-8 mb-12">
        <h3 className="text-sm font-semibold tracking-wide uppercase text-white/50 mb-8">
          Filter Transactions
        </h3>
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-6">
          ...
          <div>
            ...
            <input
              ...
              className={`${inputBase} ${
                fareFilter === "all" ? "opacity-40 cursor-not-allowed" : ""
              }`}
            />
          </div>
          ...
          <div className="overflow-x-auto h-full">
            <table className="w-full h-full text-sm">
              <thead className="bg-[#1f1f1f] text-white/60 text-xs uppercase tracking-wide sticky top-0 z-10">
                <tr>
                  <th className="px-5 py-4 text-left font-semibold min-w-[120px]">
                    Booking ID
                  </th>
                  <th className="px-5 py-4 text-left font-semibold min-w-[150px]">
                    Rider
                  </th>
                  <th className="px-5 py-4 text-left font-semibold min-w-[150px]">
                    Driver
                  </th>
                  <th className="px-5 py-4 text-left font-semibold min-w-[220px]">
                    Route
                  </th>
                  <th className="px-5 py-4 text-left font-semibold min-w-[100px]">
                    Fare (₹)
                  </th>
                  <th className="px-5 py-4 text-left font-semibold min-w-[100px]">
                    Payment Mode
              ...
              <tbody>
                {currentTransactions.map((t, idx) => (
                  <tr
                    ...
                    className={`h-20 border-t border-white/5 hover:bg-white/5 transition cursor-pointer ${
                      idx === currentTransactions.length - 1
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div
                        className="text-white/85 font-medium truncate max-w-56"
                        title={t.pickupAddress}
                      >
                        From: {t.pickupAddress}
                      </div>
                      <div
                        className="text-white/40 text-[11px] truncate max-w-56 mt-1"
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={
                          "inline-flex items-center px-3 py-1.5 rounded-full text-[11px] font-semibold border bg-[#222] text-white/60 border-white/10"
                        }
                      >
                        {t.paymentMode}
                      </span>
      ...
      <Pagination
        ...
        onPageChange={(page) =>
          setTransactionsData({ ...transactionsData, number: page - 1 })
        }
        position="relative"
        variant="dark"
      />
