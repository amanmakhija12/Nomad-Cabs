...
const ManageTransactions = () => {
  ...
  return (
    <div className="p-6 flex flex-col min-h-full text-white rounded-2xl pb-24">
      {/* Header */}
      <div className="mb-10">
          ...
          <div className="flex gap-2">
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`h-10 w-10 rounded-lg text-sm font-semibold border transition ${currentPage === i + 1 ? 'bg-white text-black border-white shadow' : 'bg-[#1b1b1b] text-white/60 border-white/10 hover:bg-[#232323]'}`}
              >{i + 1}</button>
            ))}
          </div>
          <button
